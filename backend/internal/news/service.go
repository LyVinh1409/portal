...
// CreateWithAuthor creates a news article with author ID from JWT
func (s *Service) CreateWithAuthor(ctx context.Context, authorID uint, title, excerpt, body, slug string, seoTitle, seoDesc string, featuredKey *string, publishedAt *time.Time, tagIDs []uint) (*models.News, error) {
    if slug == "" { slug = makeSlug(title) }
    news := &models.News{
        Title: title,
        Excerpt: excerpt,
